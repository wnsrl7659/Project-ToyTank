...
unsigned SoundSystem::GetSoundEventIndex(const char* path) const {
  ...
  eventDescriptions_[i]->getPath(buffer, len + 1, nullptr);
...
FMOD::Studio::EventInstance* SoundSystem::PlayFmodSound(unsigned index) {
...
void SoundSystem::StartLoopingSound(unsigned index, float* parameter) {
  //  play the sound
  EventInstancePtr instance = PlayFmodSound(index);
  //  add to the loop list
  unsigned EmptyLoopSound = FindEmpty();
  if (EmptyLoopSound >= loopingSounds_.max_size())
    printf("StartLoopingSound, Out of Index!!");
  else
...
static bool ErrCheck(FMOD_RESULT fmodErr_, const char* errMsg) {
  if (fmodErr_ != FMOD_OK) {
  Util::ServiceLocator::getLogger()->print(Util::error, errMsg);
  std::cout << fmodErr_;
  return true;
  }
  return false;
...
static void LoadBanks(StudioSystemPtr system_, std::vector<BankPtr>& banks_, FMOD_RESULT& fmodErr_) {
  banks_.resize(5); // create pointers to fill in
  ...
  fmodErr_ = system_->loadBankFile("data/audio/banks/Game.bank", FMOD_STUDIO_LOAD_BANK_NORMAL, &(banks_[2]));
  ErrCheck(fmodErr_, "FMOD error loading Master.strings.bank\n");
  fmodErr_ = system_->loadBankFile("data/audio/banks/Menu.bank", FMOD_STUDIO_LOAD_BANK_NORMAL, &(banks_[3]));
  ErrCheck(fmodErr_, "FMOD error loading Master.strings.bank\n");
  fmodErr_ = system_->loadBankFile("data/audio/banks/Music.bank", FMOD_STUDIO_LOAD_BANK_NORMAL, &(banks_[4]));
  ErrCheck(fmodErr_, "FMOD error loading Master.strings.bank\n");
}
static void LoadEventDescriptions(std::vector<BankPtr>& banks_,
          std::vector<EventDescriptionPtr>& eventDescriptions_,
